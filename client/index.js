...
const config = {
    headers: {
        ...
        'api-key': '64b428aa75f99e74c5b19472'
    }
}
...
const createUser = async() => {
    ...
    await axios.post('https://ecotone-production.up.railway.app/api/users/post', {
...
const updateUserLogin = async() => {
    ...
    const user = await axios.get(`https://ecotone-production.up.railway.app/api/users`)
    let userData = user.data.users
    console.log(userData)
    userData.forEach(async(elem)=>{
        await axios.put(`https://ecotone-production.up.railway.app/api/users/put/${elem._id}`, {
            loggedIn:false
        })
    })
    for(let i = 0;i<userData.length;i++){
        if(usernameInput === userData[i].username && emailInput === userData[i].email && passwordInput === userData[i].password){
            if(userData[i].loggedIn === false){
                await axios.put(`https://ecotone-production.up.railway.app/api/users/put/${userData[i]._id}`, {
                    loggedIn:true
                })
...
const deleteAccount = async() => {
    const users = await axios.get('https://ecotone-production.up.railway.app/api/users')
    let userData = users.data.users
    userData.forEach(async(elem)=>{
        if (elem.loggedIn === true){
            await axios.delete(`https://ecotone-production.up.railway.app/api/users/delete/${elem._id}`)
